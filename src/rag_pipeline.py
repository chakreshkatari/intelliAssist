...
class DocumentAssistant:
    ...
    def ingest_file(self, file_path: str, chunk_size: int = 800, chunk_overlap: int = 120) -> int:
        ...
        # (Re)build embeddings for all chunks. For the LSA fallback embedder this
        # ensures the TF-IDF/SVD vocabulary reflects the full corpus; for the
        # sentence-transformer embedder this is just a plain re-encode.
        ...
        self.vector_store = FaissVectorStore(dim=all_embeddings.shape[1])
        self.vector_store.add(all_embeddings, self.chunks)
    ...
    def ask(self, question: str, top_k: int = 4) -> RagAnswer:
        ...
        retrieved = self.semantic_search(question, top_k=top_k)
        ...
        citations = [
            Citation(
                ...
                score=r.score,
                text_preview=(r.chunk.text[:220] + "...") if len(r.chunk.text) > 220 else r.chunk.text,
            )
            for r in retrieved
        ]
        ...
        if not retrieved:
            return RagAnswer(
                answer=(
                    "No documents have been indexed yet, or none matched your "
                    "question. Please upload a document first."
                ),
                citations=[],
        ...
        context = "\n\n".join(
            f"[{r.chunk.doc_name} | chunk {r.chunk.chunk_index}] {r.chunk.text}" for r in retrieved
        )
    ...
    def summarize_document(self, doc_name: str, num_sentences: int = 5) -> str:
        ...
        if not text:
            return "Document not found or contains no extractable text."
        return _summarize(text, target_sentences=num_sentences)
